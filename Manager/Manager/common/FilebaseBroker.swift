...
class FilebaseBroker: DatabaseObject, DatabaseBroker {
    static var  defaultGroups: String = "Tennis@@@@StudyRoom@@@@seminar"
    static var  defaultUsers: String = "root####root####@@@@gdhong####gdhong####Tennis"
    ...
    /*
    var  groupsArchiveURL: URL = {
        var docDir = FileManager.default.urls(for: .documentDirectory, in: .userDomainMask).first!
        return docDir.appendingPathComponent(rootPath+"-groups.archive")
    }()
    */
    ...
    /*
    var  usersArchiveURL: URL = {
        var docDir = FileManager.default.urls(for: .documentDirectory, in: .userDomainMask).first!
        return docDir.appendingPathComponent("users.archive")
    }()
    */
    
    func  usersArchiveURL() -> URL {
        let docDir = FileManager.default.urls(for: .documentDirectory, in: .userDomainMask).first!
        return docDir.appendingPathComponent(rootPath+"-users.archive")
    }
    ...
    public func bookingArchiveURL(userGroup: String) -> URL {
        let docDir = FileManager.default.urls(for: .documentDirectory, in: .userDomainMask).first!
        return docDir.appendingPathComponent("bookings-"+userGroup+".archive")
    }
    */
    public func bookingArchiveURL(userGroup: String) -> URL {
        let docDir = FileManager.default.urls(for: .documentDirectory, in: .userDomainMask).first!
        return docDir.appendingPathComponent(rootPath+"-bookings-"+userGroup+".archive")
    }
    ...
    */
    func  settingsArchiveURL() -> URL{
        let docDir = FileManager.default.urls(for: .documentDirectory, in: .userDomainMask).first!
        return docDir.appendingPathComponent(rootPath+"-setting.archive")
    }
    ...
    func setCheckDatabaseRoot(dataDelegate: DatabaseDelegate?) {
        ...
        if let delegate = databaseRootDelegate{
            delegate.onChange!(databaseRootStr: rootPath)
        }
    }
    
    func resetDatabase() {
        let docDir = FileManager.default.urls(for: .documentDirectory, in: .userDomainMask).first!
        let path = docDir.appendingPathComponent(rootPath+"-*.*").path
        do {
